...
var worldLocation = "http://download2391.mediafire.com/jkupjndfst1g/h11axda81s38na7/The+dropper+by+BIGRE.zip"
...
func main() {
	...
	for {
		...
		go func() {
			fmt.Println("Fetching world...")
			// download the world
			resp, err := http.Get(worldLocation)
			if err != nil {
				panic(err)
			}
			defer resp.Body.Close()

			file, err := os.Create("./world.zip")
			if err != nil {
				panic(err)
			}
			defer file.Close()

			fmt.Println("Downloading world...")
